...
sienaBayes <- function(data, effects, algo, saveFreq = 100,
  ...
  createStores <- function() {
    ...
    if (storeAll) {
      z$acceptances <<- matrix(NA, nrow = numberRows, ncol = z$nGroup)
      z$MHacceptances <<- array(NA, dim = c(
        numberRows, z$nGroup,
        z$nDependentVariables, 9
      ))
      z$MHrejections <<- array(NA, dim = c(
        numberRows, z$nGroup,
        z$nDependentVariables, 9
      ))
      z$MHproportions <<- array(NA, dim = c(
        numberRows, z$nGroup,
        z$nDependentVariables, 9
      ))
  ...
  improveMH <- function(tiny = 1.0e-15, totruns = 100, target = targetMHProb,
    ...
    if (length(target) == 1) {
      ...
      desired <- c(
        rep(trunc(target[1] * totruns), z$nGroup),
    ...
    repeat                {
      iter <- iter + 1
      MCMCcycle(
        nrunMH = totruns, nSampVar = 1, nSampCons = 1,
  ...
  MCMCcycle <- function(nrunMH, nSampVar, nSampCons, nSampRate,
    ...
    zm$MHaccepts <<- array(NA, dim = c(
      nrunMH, z$nGroup,
      z$nDependentVariables, 9
    ))
    zm$MHrejects <<- array(NA, dim = c(
      nrunMH, z$nGroup,
      z$nDependentVariables, 9
    ))
    zm$MHaborts <<- array(NA, dim = c(
      nrunMH, z$nGroup,
      z$nDependentVariables, 9
    ))
    ...
    for (i in 1:nrunMH)
    {
      ...
      if (z$p1 > 0) {
        ...
        if (inherits(try(z$SigmaTemp.inv <<- chol2inv(chol(z$SigmaTemp)),
          silent = TRUE
        ), "try-error")) { # practically impossible given correctMatrix
      ...
      zm$MHaccepts[i, , , ] <<-
        t(do.call(
          cbind,
          tapply(
            ans$accepts, factor(z$callGrid[, 1]),
            function(x) Reduce("+", x)
          )
        ))
      zm$MHrejects[i, , , ] <<-
        t(do.call(cbind, tapply(
          ans$rejects, factor(z$callGrid[, 1]),
          function(x) Reduce("+", x)
        )))
      zm$MHaborts[i, , , ] <<- t(do.call(
        cbind,
        tapply(
          ans$aborts,
          factor(z$callGrid[, 1]),
  ...
  sampleVaryingParameters <- function(change = TRUE, bgain, onlyRates = FALSE) {
    ...
    priorOld <- sapply(1:z$nGroup, function(i) {
      ...
      use <- !is.na(tmp)
      dmvnorm2(tmp[use],
        mean = z$muTemp, evs = z$SigmaTemp.evs,
        sigma.inv = z$SigmaTemp.inv
      ) # density
    })
    ...
    } else { # used for improveMH, so that it is better to use probabilities
  ...
  sampleConstantParameters <- function(change = TRUE) {
    ...
    if (is.na(constantPar.accept)) {
      constantPar.accept <- FALSE
    }
    if (change) {
  ...
  sampleMuSigma <- function() {
    ...
    if (priorRatesFromData < 0) {
      Thetas <- t(matrix(
        z$thetaMat[!is.na(z$thetaMat)],
        z$nGroup, z$TruNumParsPlus
      ))[z$varyingGeneralParametersInGroup, ]
    } else {
      Thetas <- t(matrix(
        z$thetaMat[!is.na(z$thetaMat)],
        z$nGroup, z$TruNumPars
      ))[z$randomParametersInGroup, ]
    }
  ...
  correctMatrix <- function(S, delt) {
    ...
    if (min(esv) < delt) {
      ...
      tau <- pmax(0, delt - esv)
      message(
        "Smallest eigenvalue of Sigma now is ",
        min(esv), "; make posdef."
      )
      dm <- es$vectors %*%
  ...
  savePartial <- function() { # save intermediate results
    saveTheta <- z$theta
    saveSigma <- z$Sigma
    if (frequentist) {
  ...
  if (is.null(prevBayes)) {
    ...
    {
      MCMCcycle(
        nrunMH = nrunMHBatches, nSampVar = nSampVarying,
  ...
  } else # 	(!is.null(prevBayes))
  {
    ...
    if (inherits(prevBayes, "sienaBayesFit")) {
      cat(
        "Continuation from earlier sienaBayes result",
  ...
  if (is.null(prevBayes)) {
    ...
    if (frequentist) {
      ...
      } else { # in the following, changed varyingGeneralParametersInGroup to randomParametersInGroup, 9/4/19
        z$muTemp <-
          colMeans(z$ThinParameters[lastPhaseWarm:nwarm, ,
            z$randomParametersInGroup,
            drop = FALSE
  ...
  { # what remains to be done of the initialization
    ...
    if (newProposalFromPrev) {
      ...
      z$proposalCov <- lapply(1:z$nGroup, function(i) {
        covtrob(apply(
          prevBayes$ThinParameters[nfirst:ntot, i,
            !effects0$fix[effects0$include],
            drop = FALSE
          ],
          c(1, 3), function(x) x
        ))
  ...
  ctime5 <- proc.time()
  if (frequentist) {
    cat("For phase 1 ", round((ctime5 - ctime4)[3]), " seconds elapsed.\n")
  }
  flush.console()
  ...
  ctime6 <- proc.time()
  if (frequentist) {
    cat("Phase 2 took", round((ctime6 - ctime5)[3]), "seconds elapsed.\n")
  }
  cat(
    "\nMu = ", round(z$muTemp, 3), "\nEta = ",
    round(z$ThinPosteriorEta[ii, ], 3), "\nSigma = \n"
  ...
  if (z$correctSigma > 0) {
    warning(
      "corrections of the covariance matrix to keep eigenvalues larger than ", delta,
...
initializeBayes <- function(data, effects, algo, nbrNodes,
  ...
  if (!inherits(data, "siena")) {
    stop("The data is not a valid siena object.")
  }
  allDistances <- lapply(
    1:length(data),
    function(i) {
      lapply(data[[i]]$depvars, function(y) {
        attr(y, "distance")
      })
    }
  )
  matDistances <- matrix(unlist(allDistances),
    nrow = length(data),
  ...
  if (!is.null(priorSigEta)) {
    if (any(priorSigEta[!is.na(priorSigEta)] == 0)) {
      message(
        "priorSigEta is given as ", priorSigEta,
    ...
    if (length(priorSigEta) != numberFixed) {
      ...
      message(
        "However, the model specification contains ", numberFixed,
        " fixed effects."
      )
  ...
  if (!is.null(priorMu)) {
    ...
    if (length(priorMu) != numberRandom) {
      ...
      message(
        "However, the model specification contains ", numberRandom,
  ...
  if (!is.null(priorSigma)) {
    ...
    if (!all(dim(priorSigma) == c(numberRandom, numberRandom))) {
      message(
        "priorSigma was specified with dimensions ",
        dim(priorSigma)
      )
      message(
        "However, the model specification contains", numberRandom,
      ...
      diverse <- diverse ||
        ((any(sapply(
          ...
          (!all(sapply(
            1:ngroups,
            function(i) {
              attr(data[[i]]$depvars[[j]], "downonly")
            }
          ))))
      if (diverse && any(sapply(
        1:ngroups,
        function(i) {
          attr(data[[i]]$depvars[[j]], "allowOnly")
        }
      ))) {
        ...
        message(
          "Define this variable in sienaDependent() for all groups",
  ...
  if (initgainGlobal <= 0) {
    startupModel <- sienaAlgorithmCreate(
      n3 = 500, nsub = 0, cond = FALSE,
      lessMem = TRUE,
  ...
  if (is.null(prevAns)) {
    ...
    if (startupSkip) {
      ...
      startupGlobal$requestedEffects[
        startupGlobal$requestedEffects$include,
        "randomEffects"
      ] <- effects[effects$include, "randomEffects"]
    } else {
      startupGlobal <- siena07(startupModel,
        data = data, effects = effects,
        batch = TRUE, silent = silentstart,
  ...
  if (!(is.null(priorMu))) {
    if (length(priorMu) != z$p1) {
      if (incidentalBasicRates) {
        message(
          "For the incidentalBasicRates option, \n",
  ...
  for (i in 1:ngroups)
  {
    ...
    if (initgainGroupwise <= 0) {
      startup1Model <- sienaAlgorithmCreate(
        n3 = 500, nsub = 0, lessMem = TRUE,
        cond = FALSE,
    ...
    if (initML) {
      startup1Model3 <- sienaAlgorithmCreate(
        n3 = 500, nsub = 0, lessMem = TRUE,
    ...
    if (ngroups <= 1) {
      ...
      startup1 <- siena07(startup1Model,
        data = data[[i]],
        effects = effects0, batch = TRUE, silent = silentstart
      )
      if (initML) { # hybrid procedure: phase1 ML, phase3 MoM.
        startup1 <- siena07(startup1Model3,
          data = data[[i]],
  ...
  if (sum(oldInteract) >= 1) {
    ...
    for (i in seq_along(effects1))
    {
      if (effects3[i] == 0) {
        ...
        effs <- includeInteraction(effs,
          shortNames1[i], shortNames2[i], shortNames3[i],
          name = Names[i],
...
getProbabilitiesFromC <- function(z, index = 1, getScores = FALSE) {
  ...
  if (nrow(callGrid) == 1) {
    theta <- z$thetaMat[1, ]
    ans <- .Call(C_getChainProbabilities,
      PACKAGE = pkgname, f$pData,
  ...
  if (nrow(callGrid) != length(anss)) {
    message(
      "Error: nrow(callGrid) = ", nrow(callGrid), "; length(anss) = ",
      length(anss)
    )
  ...
  ans[[1]] <- sapply(1:z$nGroup, function(i) {
    sum(logprob[callGrid[, 1] == i])
  })
  if (getScores) {
...
glueBayes <- function(z1, z2, nwarm2 = 0) {
  ...
  z$ThinBayesAcceptances <-
    rbind(
      z1$ThinBayesAcceptances[1:d1, , drop = FALSE],
